document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (isUserAuthenticated) {
    ...
    const container = document.querySelector("#found-movies");
    ...
    if (movieData) {
      try {
        ...
        console.log("Parsovaná data:", parsedData);

        if (parsedData.results && Array.isArray(parsedData.results)) {
          console.log(`Nalezeno ${parsedData.results.length} filmů.`);
              ...
              <div class="columns-container">
                ...
                <div class="right-col">
                  <div class="text-area" id="n${movie.id}">
                    ...
                    <button class="like-button ${likeClass} n${movie.id}" data-movie-id="${movie.id}">${buttonText}</button>
                  </div>
                </div>
              </div>
            </article>
